...
class ViewsAreVisibleTest(TestCase):
    ...
    def test_can_render_room_details(self):
        MeetingsModelTest.setUpTestData()
        response = self.client.get('/rooms/1')  # I would like to improve this test to take the ID in the response
        # for a truer test, however, knowing I have just entered 1 room means I know the ID is 1 and should
        # pass as a test
        self.assertEqual(response.status_code, 200)
    # Test currently fails, BDD has currently been used but now I want to try and create room details following TDD
